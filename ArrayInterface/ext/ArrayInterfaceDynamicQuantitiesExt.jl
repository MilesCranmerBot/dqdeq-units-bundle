...
function ArrayInterface.qr_instance(A::Matrix{T}, pivot = ArrayInterface.DEFAULT_CHOLESKY_PIVOT) where {T <: DynamicQuantities.UnionAbstractQuantity}
    if pivot === ArrayInterface.DEFAULT_CHOLESKY_PIVOT
        return LinearAlgebra.QRCompactWY(Matrix{T}(undef, 0, 0), Matrix{T}(undef, 0, 0))
    else
        return LinearAlgebra.QRPivoted(Matrix{T}(undef, 0, 0), Vector{T}(undef, 0), Vector{Int}(undef, 0))
    end
end

function ArrayInterface.svd_instance(A::Matrix{T}) where {T <: DynamicQuantities.UnionAbstractQuantity}
...
function ArrayInterface.lu_instance(A::Matrix{T}) where {T <: DynamicQuantities.UnionAbstractQuantity}
